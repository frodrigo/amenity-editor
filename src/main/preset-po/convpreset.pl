...
# From http://svn.openstreetmap.org/applications/editors/josm/i18n/convpreset.pl
...
#  print "tr(\"\"); ";
  ...
  elsif($line =~ /<chunk.*\s+name=(".*?")/)
  {
    my $val = fix($1);
    $chunk = $group ? "$group$val" : $val;
    $chunk =~ s/""/\//;
    if($line =~ /name_context=(".*?")/)
    {
      print "/* chunk $chunk */ trc($1, $val);\n";
    }
    else
    {
      print "/* chunk $chunk */ tr($val);\n";
    }
  }
